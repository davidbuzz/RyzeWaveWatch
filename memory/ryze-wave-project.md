...
- 2026-09-05 16:00: Buzz's Pixel walk (> 200 m) hit 'Only 23 steps recorded' in stride calibration: the code pro-rates HOURLY step bins by time overlap (never right for short walks). Fix design = per-workout steps from B1 running-total deltas + phone TYPE_STEP_COUNTER fallback, honest messages; to run in the main checkout after the tracker fix loop finishes (a worktree attempt was stopped: see no-worktrees-without-permission). Open diagnostic: does the watch itself count steps during a sport session? (hour total looked low both walks; asked Buzz for the watch-face number.)
- 2026-09-05 16:20: history rewritten with git plumbing (working tree untouched): build 8 = 4e63e27, docs = c662627, coordinate rule = 1c28d2e; old objects purged (reflog expired + gc). No real coordinates remain in any reachable object. Rule: GPS logs get lat AND lon shifted by >= 1 km, a different pair per log (CLAUDE.md).
- 2026-09-05 17:20 run: watch DOES count steps per session (FD01 bytes 7-9 = session steps 0->3933), but does NOT add them to the hourly step bins (17:00 hour only 157) — so per-workout steps MUST come from the FD01 session field, not hourly pro-rating. App bug: watch-side pause/resume/stop (FD 22/33/00 from the watch) ignored; a watch pause at +12 s + a phone pause left the app paused 30 min, GPS off, watch km/pace frozen at 0. Manually rebuilt workout #4 on the Pixel (31:06, 3.31 km from cadence-split steps x stride since GPS was lost, avg HR 142/max 166, 501 kcal Keytel) and re-exported to Health Connect. Fixes queued: mirror both-direction pause/resume/stop as one state + spoken "workout paused/resumed"; keep GPS while paused; finalise on stop-from-paused; use FD01 session steps for calibration.
