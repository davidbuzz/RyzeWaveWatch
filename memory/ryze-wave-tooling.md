...
- Gadgetbridge sparse clone (gloryfit, no1f1, jyou dirs only): `<repo>/tools/Gadgetbridge` (moved into the repo 2026-09-04 at Buzz's request; git-ignored)
- Gadgetbridge-tools (branch `jr-gb-dissector`), Wireshark dissector at `gloryfit/gloryfit.lua`: `<repo>/tools/Gadgetbridge-tools`
- jadx 1.5.6: `<repo>/tools/jadx/bin/jadx` (moved into the repo 2026-09-04; ~/tools no longer exists)
- Ryze Fit 1.3.8 XAPK from APKPure, unpacked and decompiled: `<repo>/apk/` (`xapk/`, `jadx-out/sources`)
- Project venv with bleak: `<repo>/.venv`
