...
not a lap pool, a road test but not a lab one, on the water but not on an erg. "No" means the wearer stays indoors
or stays put, and a fix would add nothing but jitter. Note the column is independent of the step column: cycling
...
| `0x05` | Badminton | Sharp racket swings between short shuffles | Partly — steps inflated by swings | No — indoors or stays put | Yes, `ANY` |
| `0x07` | Tennis | Racket swings and serves between sprints | Partly — steps inflated by swings | No — indoors or stays put | Yes, `ANY` |
...
| `0x0A` | Basketball | Dribbling and shooting over constant footwork | Partly — steps inflated by swings | No — indoors or stays put | Yes, `ANY` |
...
| `0x0D` | Volleyball | Repeated overhead arm swings, little travel | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x12` | Spinning | Hands fixed on the bars; legs do the work unseen | **No** — nothing usable | No — indoors or stays put | Yes, `NONE` |
| `0x13` | Yoga | Slow, deliberate limb movement, held poses | **No** — nothing usable | No — indoors or stays put | Yes, `NONE` |
| `0x14` | Sit-ups | Torso curls; the wrist rises and falls each rep | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x15` | Treadmill | Swings with the stride, indoors | **Yes** — footfalls | No — indoors or stays put | Yes, `ANY` |
| `0x17` | Boating | Paddle stroke, one arm cycle per stroke | **Yes** — strokes, not steps | **Yes** — outdoors and covers ground | Yes, `STROKES` |
| `0x18` | Jumping Jacks | Arms sweep overhead once per jump | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x1B` | Indoor Running | Swings with the stride, indoors | **Yes** — footfalls | No — indoors or stays put | Yes, `ANY` |
| `0x1C` | Strength Training | Lifts and presses, one arc per repetition | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x1F` | Elliptical | Moving handles push and pull with the legs | **Yes** — strokes, not steps | No — indoors or stays put | Yes, `STROKES` |
| `0x22` | Boxing | Punches thrown in combinations | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x27` | Taekwondo | Strikes and blocks in sequence | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x2D` | Waist Training | Twisting torso work, arms following | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x2E` | Karate | Strikes and blocks in sequence | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x35` | Archery | Draw and hold, then still | **No** — nothing usable | No — indoors or stays put | Yes, `NONE` |
| `0x37` | Aerobic Combo | Choreographed arm patterns with step patterns | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x39` | Street Dancing | Fast, irregular arm movement with footwork | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x3A` | Kick Boxing | Punches and kicks in combinations | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x3F` | Handball | Throwing arm over constant running | **Yes** — footfalls | No — indoors or stays put | Yes, `ANY` |
| `0x40` | Bowling | A few approach steps, then one swing | Partly — real steps, but few | No — indoors or stays put | Yes, `ANY` |
| `0x41` | Racquetball | Racket swings in a confined court | Partly — steps inflated by swings | No — indoors or stays put | Yes, `ANY` |
...
| `0x50` | Core Training | Floor work: planks, twists, raises | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x51` | Skating | Arms swing side to side for balance | **No** — nothing usable | **Yes** — outdoors and covers ground | Yes, `NONE` |
| `0x55` | Kickboxing Aerobics | Punch and kick patterns to a beat | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x56` | Lacrosse | Cradling and passing over constant running | **Yes** — footfalls | **Yes** — outdoors and covers ground | Yes, `ANY` |
| `0x58` | Wrestling | Grappling; continuous irregular motion | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x59` | Fencing | Lunges with a still, extended sword arm | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x60` | Pickleball | Paddle swings in a small court | Partly — steps inflated by swings | No — indoors or stays put | Yes, `ANY` |
| `0x61` | HIIT | Whatever the interval is: often both | Partly — repetitions, not travel | Sometimes — outdoors intervals only | Yes, `REPS` |
| `0x62` | Shooting | Raise, hold, fire; deliberately motionless | **No** — nothing usable | No — indoors or stays put | Yes, `NONE` |
| `0x63` | Judo | Grips and throws; continuous irregular motion | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x6C` | Pull-up | One pull per repetition, wrist travels far | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
| `0x6D` | Push-up | One press per repetition, wrist on the floor | Partly — repetitions, not travel | No — indoors or stays put | Yes, `REPS` |
...
| `0x71` | Bungee Jumping | A fall, then hanging; nothing rhythmic | **No** — nothing usable | No — indoors or stays put | Yes, `NONE` |
| `0x72` | Long Jump | A short run-up, then a single jump | Partly — real steps, but few | No — indoors or stays put | Yes, `ANY` |
