...
2. drop fixes with accuracy worse than 60 m (was ~20 m, which gave 0 km on a poor-signal run), and ignore movement below 1.5 × the accuracy radius when speed ≈ 0; do not *start* on a poor fix: wait up to 15 s for one of ≤ 20 m before anchoring (the first real walk anchored on a 52 m fix 45 m off the route and counted the hop back), but credit the Doppler integral over the wait on every way out of it (the movement was real, only its start was doubted — dropping it cost 45 m per start and per resume at 3 m/s) and never end the wait in a rejection (an implausible fix at expiry anchors instead of deadlocking); let a ≥ 3× better fix inside a poor *starting* anchor's radius (nothing credited from it yet) replace it, crediting only the Doppler integral — a poor anchor mid-run is the last accepted fix of a run in progress and is kept, or replacing it drops real movement on every accuracy step;
3. accumulate distance between accepted fixes: with a Doppler speed ≥ 1 m/s the integral of each fix's reported speed × its own dt since the last accepted fix (capped by the haversine hop + accuracy radius, only while fixes are ≤ 5 s apart; spikes contribute nothing; the sub-threshold part is believed only as far as the hop confirms it, so a receiver saying 0.7 m/s while standing costs the hop, not an accuracy radius per stop), otherwise the haversine hop; reject implausible jumps (> 12 m/s, or beyond max(speed, 2.5 m/s, the last accepted speed) × dt + both radii — without the last term a speed-less receiver deadlocked at 6 m/s), and if spikes go on for 10 s (or three agree with each other) re-anchor on the current fix without credit rather than lose the rest of the workout. Implemented in `DefaultGpsDistanceTracker` (docs/APP.md "Tracker refinements (build 14)" and its "second review" subsection: real walk 155 → 131 m, synthetic 20-minute runs within 0.1 % with Doppler, 1.5 % without, also with the accuracy stepping between bands, at 6–7 m/s without Doppler, and with a creeping receiver);
