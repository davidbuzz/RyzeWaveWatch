...
| `FD 11 <type> <ivl>` / `FD 22 <type> <ivl> <dur> cal16 …` (13 B) / `FD 00 <type> <ivl>` | workout start / pause / stop; watch echoes the same bytes | V | **Watch-originated presses (pause/play on the watch's paused screen) and the watch's echo of the app's resume come in the 13-byte form `FD <op> <type> <ivl> <dur> cal16 …` for every op** (2026-09-05: six 13-byte `FD 33`, seven 13-byte `FD 22`); the app's own echoes are 4 B. `type` = sport id from the watch's own list (§6c; 1 = Outdoor Running, 0x23 = Outdoor Walking, 2 = Cycling — all three started the matching mode on the wrist 2026-09-04/05); ivl = HR report interval s; `FD 33` resume is SDK-documented, not captured. The watch also sends these **unsolicited** when the user presses pause/resume/stop **on the watch** (not just as an echo of a phone command): the app tells the two apart by a short expected-echo window after its own send and drives its own pause/resume/stop from a watch-originated one (`WatchApiImpl` → `WatchEvent.WorkoutControl`), without echoing it back. **The watch answers the app's `FD 00 <type> 01` stop with a 4-byte `FD 11 <type> 01`** (verified 2026-09-11, 3/3 stops, ~90-200 ms later) — a stop handshake, not a start press; the app must ignore an `FD 11` this close to its own stop or every stop restarts the workout (`WorkoutController` debounces it). **The 13-byte body is the app's own last `FD 44` replayed back — the watch keeps no workout clock of its own** [V] (hardware, 2026-09-15, moto g05 + RyzeBridge, captures/bridge_20260915_1824*.txt): we pushed `FD 44 01 01 00 37 37 …` (55:55), sent nothing for 11 s, then sent a bare 4-byte `FD 22 01 01`; the watch answered `FD 22 01 01 00 37 37 …` — byte-identical to our `FD 44` but for the opcode, still 55:55, not the ~20 s the session had really been running. A bare `FD 33` answered the same. The elapsed-time screen is a pure mirror of the last `FD 44`: it follows arbitrary jumps forward (`00 0a 00` = 10:00) and **backward** (`00 00 1e` = 0:30) on command, and freezes when the pushes stop (confirmed on the wrist by Buzz, same session). Two consequences. (1) `<dur>` in a watch-originated packet carries **no information the phone did not just supply**, so it can NOT be used to tell a genuine wrist press from the watch's junk `FD 22`/`FD 33` flood — measured over all 35 unsolicited 13-byte packets in `captures/pixel_run{,2}_*`, genuine presses differ from the phone's own elapsed by 0-26 s and junk by 0-17 s, i.e. junk is a strict *subset* of genuine and no threshold separates them. (2) Because the app keeps pushing `FD 44` for the 8 s its debounce ignores a wrist pause, the watch's locally-frozen display is overwritten by newer numbers, so a 2 s wrist pause shows up on the face as the time standing still and then **jumping forward** (Buzz, 2026-09-15). Encoding caveat: the app sends `<dur>` as `hh mm ss` (`Protocol.encSportUpdate`; verified over 2638 packets, rollover `00 00 3b` → `00 01 00`), but the watch re-encodes it in **unsolicited** packets as a big-endian second count — our `00 01 29` (1 m 41 s) came back as `00 00 64` (100) — so a byte above 59 is normal there and a single fixed decoder is wrong for one of the two directions |
